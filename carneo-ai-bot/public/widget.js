(function(){
  ...
  bubble.style.cssText = 'position:fixed;bottom:20px;right:20px;width:64px;height:64px;border-radius:999px;background:#111;color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:999999;box-shadow:0 8px 24px rgba(0,0,0,.2)';
  bubble.innerText = 'AI';
  document.body.appendChild(bubble);

  const panel = document.createElement('div');
  panel.style.cssText = 'position:fixed;bottom:100px;right:20px;width:360px;height:520px;background:#fff;border-radius:16px;box-shadow:0 16px 48px rgba(0,0,0,.25);display:none;flex-direction:column;overflow:hidden;z-index:999999';
  panel.innerHTML = `
  ...
  function addMsg(text, who){
    ...
    div.innerHTML = `<div style="font-weight:600;color:${who==='you'?'#111':'#555'}">${who==='you'?'Vy':'Carneo AI'}</div><div>${text}</div>`;
